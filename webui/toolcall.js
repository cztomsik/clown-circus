...
// rendered separately (and unchanged) by Message.js. Every tool registered in
...
const ReadFile = ({ a }) => html`
  <div class=${HEAD}>
    <span class="text-dim">read </span><span class="text-ink">${a.path ?? ''}</span>
    ${a.raw ? html` <span class="text-dim/70">· raw</span>` : null}
  </div>`;
  ...
  return html`
    <div class="space-y-1">
      <div class=${HEAD}>
        ...
        ${lines ? html` <span class="text-dim/70">· ${lines} line${lines === 1 ? '' : 's'}</span>` : null}
      </div>
      ${c ? html`<pre class=${`${PRE} ${SCROLL}`}>${c}</pre>` : null}
    </div>`;
};
  ...
  return html`
    <div class="space-y-1">
      <div class=${HEAD}>
        ...
        ${a.replace_all ? html` <span class="text-dim/70">· all</span>` : null}
      </div>
...
const RunCommand = ({ a }) => html`
  <div class="space-y-1">
    <pre class=${PRE}><span class="text-accent">$ </span><span class="text-ink">${a.command ?? ''}</span></pre>
    ${a.cwd ? html`<div class="px-2 text-xs font-mono break-all"><span class="text-dim/70">cwd </span><span class="text-dim">${a.cwd}</span></div>` : null}
  </div>`;
  ...
  return html`
    <div class="space-y-1">
      <div class=${HEAD}>
        ...
        ${done ? html` <span class="text-dim/70">· ${done} done</span>` : null}
      </div>
      ${tasks.length ? html`
        <ul class="m-0 px-2 py-1.5 text-xs list-none space-y-0.5 rounded bg-line/50 ${SCROLL}">
          ${items.map((t, i) => t.done !== undefined
            ? html`<li key=${i} class=${`break-words ${t.done ? 'text-dim/60 line-through' : t.inProgress ? 'text-accent font-medium' : 'text-dim'}`}>${t.done ? '✓' : '•'} ${t.text}</li>`
            : html`<li key=${i} class="text-dim/70 break-words">${t.text}</li>`)}
        </ul>` : null}
    </div>`;
};

// load_skill — just the skill name.
const LoadSkill = ({ a }) => html`
  <div class=${HEAD}>
...
const Generic = ({ raw }) => html`<pre class=${PRE}>${raw ?? ''}</pre>`;
...
export const ToolCall = ({ name, args, raw }) => {
  ...
  return R && args ? html`<${R} a=${args} />` : html`<${Generic} raw=${raw} />`;
};
