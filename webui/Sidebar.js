...
const SessionItem = ({ s, active, onSelect }) => html`
  <li class=${active
    ...
    <span class="flex items-center gap-1.5 ${s.archived ? 'text-dim' : ''}">
      ...
      ${s.archived ? html`<span class="text-[9px] uppercase tracking-wider text-dim border border-line rounded px-1 flex-none">archived</span>` : null}
    </span>
    ...
    <ul class="list-none m-0 p-0 overflow-y-auto flex-1">
      ...
      ${archived.length ? html`
        <${Fragment} key="archived">
          ...
          ${archived.map((s) => html`<${SessionItem} key=${s.id} s=${s} active=${s.id === current} onSelect=${onSelect} />`)}
        </${Fragment}>
      ` : null}
    </ul>`;
};
