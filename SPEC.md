...
| `POST` | `/sessions/:id/trim`       | `/trim [n]`     | Trim the transcript, **keeping the last n rounds untouched**: in every round *before* the kept tail, truncate any `role=tool` result longer than 1024 bytes to `[tool result truncated — was <N> bytes]` and delete both `reasoning_content` and `reasoning` from assistant messages. A round is a user message + the assistant/tool messages it triggers; the kept tail is everything from the (total-n+1)-th user message on. The kept tail is untouched, as are user text, assistant text, and the tool-call invocations (name + arguments) — only the tool *results* and the CoT go. `n` may exceed the round count (a no-op) or be `0` (trim everything). Idempotent (a re-run is a no-op). Body `{ "keep": n }` with `n` a non-negative integer — `400` otherwise. `200` `{ "keep", "trimmed", "truncatedResults", "reasoningRemoved" }`. |
