...
- **Tech stack**: Node.js 24.x, plain JavaScript (ESM, `"type": "module"`), no TypeScript, no build step.
- **Run command**: `node src/main.js` (or `npm start`). No build/test toolchain.
- **Dependency**: `express` only (v5). Everything else — SQLite, crypto, http, child_process — is built into Node.
...
- **Snapshot interop**: the `snapshot` column stores `{ messages, todos, total_tokens }` — the **exact JSON shape** of `clown-code`'s `session-*.json`. So `GET /sessions/:id` output can be saved to a file and opened in `clown-code`, and `POST /sessions { import }` reads one back.
...
- **Error mapping**: LLM timeout → `504 llm_timeout`; unreachable/HTTP error → `502 llm_unavailable`; unknown id/import → `404`; busy → `409`; malformed/missing `cwd` → `400`; bad `cwd` dir + unhandled → `500`.
